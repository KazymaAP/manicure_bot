...
def setup_final_features_router(container: Container) -> Router:
    ...
    async def cmd_nearest_slots(message: Message) -> None:
        """FIXED: фича #5 — ближайшие 5 свободных слотов с показом до 3 слотов на дату (п.22)."""
        ...
        try:
            ...
            for date_str in available_dates[:5]:
                # БАГ 22 FIX: показываем до 3 ближайших слотов на каждую дату
                slot_objs = await sched_service.get_available_slots(date_str)
                times = [s.time for s in slot_objs[:3]]
                if times:
                    slots.append((date_str, ", ".join(times)))
    ...
    async def view_schedule_for_client(message: Message) -> None:
        """FIXED: фича #46 — просмотр расписания. БАГ 21 FIX: добавлена кнопка записи."""
        if message.from_user is None:
            return
        try:
            available_dates = await sched_service.get_available_dates_async()
            ...
            # БАГ 21 FIX: добавляем кнопку «Записаться» после показа расписания
            from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
            book_kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="💅 Записаться", callback_data="book_again_start")]
            ])
            await message.answer(text, reply_markup=book_kb)
        except Exception as exc:
