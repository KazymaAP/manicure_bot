...
from src.config.config_writer import CONFIG_JSON_PATH, save_config
...
def setup_admin_router(container: Container) -> Router:  # noqa: C901
    ...
    async def admin_apply_template_date_input(message: Message, state: FSMContext) -> None:
        ...
        try:
            ...
            # БАГ 22/23 FIX: добавлены кнопки возврата к шаблонам и просмотра расписания
    ...
    def _get_master_name(config: dict | None = None) -> str:
        """Читает имя мастера из config.json.

        ПРОБЛЕМА 11 FIX: принимает уже загруженный config как параметр,
        чтобы избежать повторного чтения конфига в _build_settings_dict.
        """
        try:
            if config is None:
                config = _load_config()
    ...
    def _build_settings_dict() -> dict:
        ...
        ПРОБЛЕМА 11 FIX: вызываем _load_config() один раз и передаём в _get_master_name.
        БАГ 2 FIX: slot_interval читается из config.json вместо захардкоженного "60".
        """
        config = _load_config()
        master_name = _get_master_name(config)
        slot_interval = config.get("schedule", {}).get("slot_interval_minutes", 60)
        return {
            ...
            "slot_interval": str(slot_interval),
    ...
    async def admin_cancel_all_date_legacy(callback: CallbackQuery, state: FSMContext) -> None:
        """Обратная совместимость — перенаправляет на новый хендлер (пункт 3 аудита)."""
        await admin_cancel_all_date_cb(callback, state)
