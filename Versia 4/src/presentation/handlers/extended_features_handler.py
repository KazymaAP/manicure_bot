...
def setup_extended_features_router(container: Container) -> Router:
    ...
    async def admin_search_client_history(message: Message, state: FSMContext) -> None:
        ...
        # БАГ 26 FIX: ранний возврат если сообщение не текстовое
        if not message.text:
        ...
        try:
            ...
            logger.error("Client history error: %s", exc)
            await message.answer("❌ Ошибка при получении истории клиента")
        finally:
            # БАГ 4 FIX: state.clear() гарантированно вызывается, даже при исключении
            await state.clear()
    ...
    async def admin_manage_templates(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        # БАГ 19 FIX: проверка типа msg перед edit_text (InaccessibleMessage не имеет edit_text)
        msg = callback.message
        if not isinstance(msg, Message):
            await callback.answer()
            return
