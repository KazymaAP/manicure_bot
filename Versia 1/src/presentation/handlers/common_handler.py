...
def setup_common_router(container: Container) -> Router:
    ...
    async def _check_subscription(user_id: int, bot: Bot) -> bool:
    ...
    # BAG-12 FIX: общая логика отправки главного меню вынесена в приватную функцию,
    # чтобы не дублировать её между main_menu_button и inline_main_menu.
    async def _send_main_menu(user_id: int, send_fn, state: FSMContext) -> None:
        """Очищает FSM и отправляет главное меню через переданный send_fn."""
        await state.clear()
        is_admin = user_id in settings.admin_ids
        portfolio = _get_portfolio(settings)
        await send_fn(
    ...
    async def main_menu_button(message: Message, state: FSMContext) -> None:
        await _send_main_menu(message.from_user.id, message.answer, state)
    ...
    async def inline_main_menu(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        if isinstance(callback.message, Message):
            await _send_main_menu(callback.from_user.id, callback.message.answer, state)
        await callback.answer()
