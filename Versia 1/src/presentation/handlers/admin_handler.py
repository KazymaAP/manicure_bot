...
from aiogram.exceptions import TelegramRetryAfter  # BAG-13 FIX: rate-limiting при рассылке
...
def setup_admin_router(container: Container) -> Router:  # noqa: C901
    ...
    def _get_master_name() -> str:
        ...
        try:
            config = _load_config()
            # BAG-18 FIX: явное приведение к str, чтобы mypy не ругался на Any
            name = config.get("master", {}).get("name", "Мастер")
            return str(name) if name else "Мастер"
        except Exception as exc:
    ...
    def _build_settings_dict() -> dict:
        ...
        """
        # BAG-15 FIX: сортируем слоты перед взятием первого и последнего,
        # т.к. администратор мог добавлять их в произвольном порядке.
        sorted_slots = sorted(settings.default_time_slots) if settings.default_time_slots else []
        return {
            "master_name": _get_master_name(),
            "welcome_text": ("настроен" if settings.services else "по умолчанию"),
            "work_hours": (
                f"{sorted_slots[0] if sorted_slots else '09:00'} - "
                f"{sorted_slots[-1] if sorted_slots else '18:00'}"
            ),
            "slot_interval": str(_load_config().get("schedule", {}).get("slot_interval_minutes", 60)),
    ...
    async def admin_edit_welcome(callback: CallbackQuery, state: FSMContext) -> None:
        if not _is_admin(callback.from_user.id):
            await callback.answer()
            return
        # BAG-10 FIX: используем _load_config() вместо дублирующего прямого open()+json.load()
        try:
    ...
    async def admin_service_duration_save(message: Message, state: FSMContext) -> None:
        ...
        try:
            ...
            # BAG-26 FIX: синхронизируем _service_durations в AppointmentService,
            # иначе новая услуга будет бронироваться с длительностью 0 (нет multi-slot).
            appt_service._service_durations[new_name] = duration
            if action == "edit" and old_name and old_name != new_name:
                appt_service._service_durations.pop(old_name, None)
        except Exception as exc:
    ...
    async def admin_blacklist_action(message: Message, state: FSMContext) -> None:
        ...
        try:
            ...
            # BAG-14 FIX: не раскрываем внутренние детали ошибки пользователю
            logger.error("Ошибка изменения черного списка: %s", exc)
            await message.answer(MessageFormatter.error_general(), reply_markup=AdminKeyboard.main_menu())
        await state.clear()
    ...
    def _calc_revenue() -> dict:  # noqa: FUNC_IN_FUNC  # BAG-16: вложенная функция, тест через to_thread
        ...
        try:
            ...
            def _sum(appts: list) -> int:  # BAG-20 FIX: добавлена аннотация типов
                total = 0
                for a in appts:
    ...
    async def admin_broadcast_send(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        async def _do_broadcast() -> None:  # BAG-21 FIX: явная аннотация возвращаемого типа
            try:
                ...
                for uid in all_user_ids:
                    try:
                        ...
                        # BAG-13 FIX: обрабатываем rate-limit отдельно с retry
