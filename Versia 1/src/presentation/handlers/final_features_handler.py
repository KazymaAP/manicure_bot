...
def setup_final_features_router(container: Container) -> Router:
    ...
    async def _toggle_notification_field(
        ...
        # BAG-6 FIX: перечитываем актуальные данные из БД вместо локального dict-слияния.
        # Если _update_user_notif поглотил ошибку, клавиатура покажет реальное состояние БД.
        updated = await asyncio.to_thread(appt_service.get_user_notification_settings, user_id)
