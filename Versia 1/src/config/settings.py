...
class Settings(BaseSettings):
    ...
    services: dict[str, dict[str, object]] = Field(
    ...
    @classmethod
    def validate_services(cls, v: dict[str, object]) -> dict[str, object]:
    ...
    # ─── Бизнес-логика ───────────────────────────────────────
    # BAG-24 FIX: поле slot_interval_minutes добавлено для хранения интервала слотов.
    # Администратор может менять его через ⚙️ Настройки → Интервал записи.
    # Значение сохраняется в config.json как schedule.slot_interval_minutes,
    # при перезапуске загружается из .env (или остаётся дефолтным 60 мин).
    slot_interval_minutes: int = Field(
        default=60,
        ge=15,
        le=480,
        description="Интервал между слотами в минутах",
    )
