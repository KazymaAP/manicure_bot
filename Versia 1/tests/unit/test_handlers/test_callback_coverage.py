...
DYNAMIC_CALLBACK_PREFIXES = [
    ...
    "admin_cancel_abort:",   # BAG-1 FIX: хендлер admin_cancel_abort в admin_handler.py
    "cancel_appt:",           # BAG-23 FIX: кнопка BookingKeyboard.cancel_appointment_list()
]
...
class TestDynamicCallbackHandlersExist:
    """BAG-22 FIX: проверяет что каждый динамический callback-prefix
    действительно имеет соответствующий startswith-хендлер в исходниках.
    """

    import os as _os
    import pathlib as _pathlib

    HANDLERS_DIR = _pathlib.Path(__file__).parent.parent.parent.parent / "src" / "presentation" / "handlers"

    def _read_all_handlers(self) -> str:
        """Читает содержимое всех файлов хендлеров."""
        content = []
        for path in self.HANDLERS_DIR.glob("*.py"):
            content.append(path.read_text(encoding="utf-8"))
        return "\n".join(content)

    def test_all_dynamic_prefixes_have_handlers(self):
        """Каждый prefix из DYNAMIC_CALLBACK_PREFIXES должен присутствовать
        как аргумент startswith() или F.data.startswith() в исходниках хендлеров.
        """
        source = self._read_all_handlers()
        missing = []
        for prefix in DYNAMIC_CALLBACK_PREFIXES:
            # Ищем строку вида startswith("prefix") или startswith('prefix')
            needle_dq = f'startswith("{prefix}")'
            needle_sq = f"startswith('{prefix}')"
            if needle_dq not in source and needle_sq not in source:
                missing.append(prefix)
        assert missing == [], (
            f"Следующие динамические callback-prefix не имеют зарегистрированных "
            f"startswith-хендлеров: {missing}"
        )
