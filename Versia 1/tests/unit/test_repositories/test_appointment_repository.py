...
class TestGetAllUserIds:
    """BAG-25 FIX: функциональный тест метода get_all_user_ids."""

    def test_get_all_user_ids_returns_list(self, repo: AppointmentRepository) -> None:
        """get_all_user_ids должен возвращать список."""
        user_ids = repo.get_all_user_ids()
        assert isinstance(user_ids, list)

    def test_get_all_user_ids_empty_initially(self, repo: AppointmentRepository) -> None:
        """При отсутствии записей список может быть пустым."""
        user_ids = repo.get_all_user_ids()
        assert isinstance(user_ids, list)

    def test_get_all_user_ids_contains_created_user(self, repo: AppointmentRepository) -> None:
        """После создания записи user_id должен появиться в списке."""
        repo.create(make_appointment(user_id=42001))
        user_ids = repo.get_all_user_ids()
        assert 42001 in user_ids

    def test_get_all_user_ids_no_duplicates(self, repo: AppointmentRepository) -> None:
        """Список не должен содержать дубликатов одного user_id."""
        repo.create(make_appointment(user_id=42002, time="10:00"))
        repo.create(make_appointment(user_id=42002, time="11:00"))
        user_ids = repo.get_all_user_ids()
        assert user_ids.count(42002) == 1
