...
class ReminderService:
    ...
    def restore_reminders(self) -> None:
        # FIX #16: очищаем устаревшие reminder_ jobs перед восстановлением
        # предотвращает накопление неактивных job-объектов в MemoryJobStore
        try:
            for job in self._scheduler.get_jobs():
                if job.id.startswith(_JOB_PREFIX):
                    with contextlib.suppress(Exception):
                        self._scheduler.remove_job(job.id)
            logger.debug("Cleared stale reminder jobs before restore.")
        except Exception as exc:
