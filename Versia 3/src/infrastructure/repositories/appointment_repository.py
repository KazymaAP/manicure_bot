...
class AppointmentRepository(BaseRepository):
    ...
    def delete_by_ids(self, appointment_ids: list[int]) -> int:
        ...
        FIX #8: батчинг по 500 записей — предотвращает SQLITE_LIMIT_VARIABLE_NUMBER.
        ...
        if not appointment_ids:
            return 0
        total_deleted = 0
        BATCH_SIZE = 500  # SQLite default limit = 999 variables
        for i in range(0, len(appointment_ids), BATCH_SIZE):
            batch = appointment_ids[i:i + BATCH_SIZE]
            ...
            with self._db.transaction() as conn:
                ...
                total_deleted += int(row[0]) if row else 0
