...
def setup_final_features_router(container: Container) -> Router:
    ...
    async def cmd_nearest_slots(message: Message) -> None:
        ...
        try:
            ...
            # FIX #22: показываем кнопку «Записаться» вместо CalendarKeyboard
            # которая вводила в заблуждение (команда /slots = список слотов, не календарь)
            kb = BookingKeyboard.book_again()
