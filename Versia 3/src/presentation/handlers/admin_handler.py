...
# ── Модульный set задач рассылки (FIX #4: перенесён на уровень модуля) ────────
...
def setup_admin_router(container: Container) -> Router:  # noqa: C901
    ...
    async def cmd_admin(message: Message) -> None:
        # FIX #2: guard на from_user is None
    ...
    async def admin_today(message: Message) -> None:
        ...
        for appt in appointments:
            if not getattr(appt, 'is_cancelled', False):
                ...
                await message.answer(
                    f"🕐 <b>{appt.time}</b> — {_html_escape(appt.client_name)}",
                    reply_markup=kb,
                    parse_mode="HTML",
                )
    ...
    async def admin_mark_arrived(callback: CallbackQuery) -> None:
        ...
        if not _is_admin(callback.from_user.id):
            await callback.answer()
            return
        # FIX #5: валидация appt_id
        try:
            appt_id = int(callback.data.split(":")[1])
            if not (0 < appt_id < 2_147_483_647):
        ...
        try:
            ...
            if appt:
                ...
                await callback.message.edit_text(
                    f"✅ {_html_escape(appt.client_name)} ({appt.time}) — отмечена как пришедшая",
                    reply_markup=None,
                )
    ...
    async def admin_cancel_request_cb(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.edit_text(
            ...
            f"Клиент: <b>{_html_escape(appt.client_name)}</b>\n"
    ...
    async def admin_slot_info(callback: CallbackQuery) -> None:
        ...
        try:
            ...
            if slot.is_booked:
                ...
                if appt:
                    await callback.answer(
                        f"📌 Занят: {_html_escape(appt.client_name)}\n📞 {_html_escape(appt.phone)}",
                        show_alert=True,
                    )
                else:
    ...
    async def admin_back_clients(callback: CallbackQuery) -> None:
        # FIX #3+#7: guard на from_user is None + проверка прав
        if callback.from_user is None:
            await callback.answer()
            return
        if not _is_admin(callback.from_user.id):
    ...
    # FIX #9: заменяем прямой open() на _load_config()
    ...
    async def admin_delete_service_confirm(callback: CallbackQuery) -> None:
        ...
        if not _is_admin(callback.from_user.id):
            await callback.answer()
            return
        # FIX #18: валидация имени услуги
        svc_name = callback.data.split(":", 1)[1] if ":" in callback.data else ""
        if not svc_name or len(svc_name) > 100:
            await callback.answer("❌ Некорректное название услуги", show_alert=True)
            return
        try:
            config = await asyncio.to_thread(_load_config)
    ...
    async def admin_broadcast_send(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        async def _do_broadcast() -> None:
            # FIX #14: дополнительная проверка текста внутри _do_broadcast
            if not text or not isinstance(text, str) or len(text) > 4096:
                logger.warning("Broadcast text invalid or too long: %r", type(text))
                return
            try:
