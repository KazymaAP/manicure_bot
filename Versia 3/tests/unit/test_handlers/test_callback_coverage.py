...
ADMIN_CALLBACKS = [
    ...
    "admin_monthly_stats",
    "admin_export_csv",
    # FIX #27: новые callback_data (добавлены при аудите 2026-06)
    "admin_back_clients",  # теперь с проверкой прав
]
...
class TestAuditFixes2026:
    """FIX #27: тесты покрытия для исправлений аудита 2026-06."""

    def test_booking_reminder_actions_has_transfer_button(self):
        """FIX #21: кнопка Перенести должна быть в reminder_actions."""
        from src.presentation.keyboards.booking import BookingKeyboard
        kb = BookingKeyboard.reminder_actions(appointment_id=1)
        assert isinstance(kb, InlineKeyboardMarkup)
        ...
        assert "transfer_appt:1" in all_cbs, "FIX #21: кнопка transfer_appt отсутствует"
        assert "reminder_yes:1" in all_cbs, "кнопка reminder_yes отсутствует"
        assert "reminder_no:1" in all_cbs, "кнопка reminder_no отсутствует"

    def test_delete_by_ids_method_exists(self):
        """FIX #8: батчинг в delete_by_ids."""
        from src.infrastructure.repositories.appointment_repository import AppointmentRepository
        assert hasattr(AppointmentRepository, "delete_by_ids")
        assert callable(AppointmentRepository.delete_by_ids)

    def test_admin_middleware_exists(self):
        """FIX #12: AdminOnlyMiddleware создан."""
        from src.presentation.middlewares.admin_middleware import AdminOnlyMiddleware
        assert AdminOnlyMiddleware is not None
        # Проверяем что принимает admin_ids
        mw = AdminOnlyMiddleware([123456789])
        assert 123456789 in mw.admin_ids

    def test_help_text_contains_new_commands(self):
        """FIX #24: help_text содержит новые команды."""
        from src.presentation.formatters.message_formatter import MessageFormatter
        text = MessageFormatter.help_text()
        assert "/mybookings" in text, "FIX #24: /mybookings не в help_text"
        assert "/slots" in text, "FIX #24: /slots не в help_text"

    def test_new_admin_callback_prefixes_coverage(self):
        """FIX #27: проверяем наличие callback-хендлеров для новых callback_data."""
        import inspect
        from src.presentation.handlers import admin_handler
        source = inspect.getsource(admin_handler)
        # Динамические callbacks
        assert "admin_slot_info:" in source
        assert "admin_toggle_slot:" in source
        assert "admin_cancel_request:" in source
        assert "transfer_cal_prev:" in source or "transfer_cal_next:" in source
        assert "time:" in source
        assert "admin_del_tmpl_confirm:" in source
        assert "admin_apply_tmpl_exec:" in source
