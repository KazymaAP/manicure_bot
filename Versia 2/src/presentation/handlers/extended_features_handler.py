...
def setup_extended_features_router(container: Container) -> Router:
    ...
    async def admin_search_client_history(message: Message, state: FSMContext) -> None:
        ...
        # BUG 16 FIX: ограничение результатов до 20 для предотвращения превышения лимита
        # Telegram 4096 символов. При большом количестве записей сообщение могло упасть с ошибкой.
        ...
        # BUG 16 FIX: показываем счётчик если результатов больше 20
        if total_results > 20:
            text += f"\n📊 <i>Показано 20 из {total_results} записей. Уточните запрос.</i>"
