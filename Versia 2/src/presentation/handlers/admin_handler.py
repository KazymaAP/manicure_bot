...
def setup_admin_router(container: Container) -> Router:  # noqa: C901
    ...
    async def _save_config(config: dict) -> None:
        ...
        def _write_config_sync(cfg: dict, path: str) -> None:
            """Синхронная запись config.json через временный файл (атомарность)."""
            ...
            try:
                with tempfile.NamedTemporaryFile(
                    ...
                    tmp_path = tmp_f.name
                    json.dump(cfg, tmp_f, ensure_ascii=False, indent=2)
        ...
        async with _config_write_lock:
            # BUG 14 FIX: блокирующие I/O операции (json.dump, os.replace) выполняются
            # в пуле потоков через asyncio.to_thread, чтобы не блокировать event loop.
            await asyncio.to_thread(_write_config_sync, config, config_path)
            try:
    ...
    async def admin_today(message: Message) -> None:
        ...
        for appt in appointments:
            if not getattr(appt, 'is_cancelled', False):
                # BUG 9 FIX: используем готовый метод AdminKeyboard.today_appointment_actions()
                # вместо ручного создания InlineKeyboardMarkup — DRY-принцип.
                await message.answer(
                    f"🕐 <b>{appt.time}</b> — {appt.client_name}",
                    reply_markup=AdminKeyboard.today_appointment_actions(appt.id, appt.client_name),
                    parse_mode="HTML",
                )
    ...
    async def admin_mark_arrived(callback: CallbackQuery) -> None:
        ...
        if not _is_admin(callback.from_user.id):
            ...
            return
        # BUG 10 FIX: защита от ValueError при некорректном callback_data
        try:
    ...
    async def _show_cancel_confirm(message: Message, appt: object, appt_id: int) -> None:
        """BUG 7 FIX: вспомогательная функция для показа диалога подтверждения отмены.
        Устраняет дублирование кода между двумя путями отмены записи:
        (1) через ввод ID с клавиатуры и (2) через инлайн-кнопку.
        """
        from html import escape as _esc
        await message.edit_text(
            f"⚠️ <b>Подтверждение отмены</b>\n\n"
            f"Клиент: <b>{_esc(getattr(appt, 'client_name', '?'))}</b>\n"
            f"Дата: <b>{_esc(getattr(appt, 'date', '?'))}</b>  "
            f"Время: <b>{_esc(getattr(appt, 'time', '?'))}</b>\n\n"
    ...
    async def admin_cancel_request_cb(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        BUG 3 FIX: явно сбрасываем FSM-состояние перед показом диалога, чтобы
        устранить конфликт с состоянием confirming_cancel из другого пути (ввод ID).
        """
        if not _is_admin(callback.from_user.id):
            await callback.answer("Нет прав администратора.", show_alert=True)
            return
        # BUG 3 FIX: сбрасываем любое текущее FSM-состояние (например, confirming_cancel
        # из пути «ввод ID»), чтобы не было конфликта двух путей отмены записи.
        await state.clear()
        try:
            appt_id = int(callback.data.split(":")[1])
        except (ValueError, IndexError):
        ...
        if not appt:
            ...
            return
        # BUG 7 FIX: используем вспомогательную функцию _show_cancel_confirm()
        # вместо дублирующего edit_text с идентичным HTML-шаблоном.
        msg = callback.message
        if isinstance(msg, Message):
            await _show_cancel_confirm(msg, appt, appt_id)
        await callback.answer()
    ...
    async def admin_slot_info(callback: CallbackQuery) -> None:
        ...
        try:
            ...
            if slot.is_booked:
                ...
                if appt:
                    # BUG 11 FIX: персональные данные клиента передаём через message.answer(),
                    # а не callback.answer() — данные не попадают в Telegram-логи,
                    # нет ограничения 200 символов, нет риска записи в серверные логи Telegram.
