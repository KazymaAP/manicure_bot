...
def setup_common_router(container: Container) -> Router:
    ...
    # BUG 6 FIX: убрана внутренняя обёртка _check_subscription — теперь везде
    # используется публичная функция check_subscription(user_id, bot, settings)
    # напрямую без дублирующей обёртки с захваченным settings.
    ...
    async def cmd_start(message: Message) -> None:
        ...
        is_subscribed = await check_subscription(user_id, message.bot, settings)
    ...
    async def check_subscription_cb(callback: CallbackQuery) -> None:
        ...
        is_subscribed = await check_subscription(user_id, callback.bot, settings)
        if is_subscribed:
            is_admin = user_id in settings.admin_ids
            portfolio = _get_portfolio(settings)
    ...
    # ── /status — диагностика для администраторов ─────────────────────────
    @router.message(Command("status"))
    async def cmd_status(message: Message) -> None:
        """BUG 17 FIX: команда /status для диагностики бота (только для администраторов).
        Показывает версию, статус БД, количество активных записей на сегодня.
        """
        user_id = message.from_user.id
        ...
        try:
            ...
            total_active = stats.get("confirmed", 0)
        except Exception as exc:
            db_status = f"❌ Ошибка: {exc}"

        now = datetime.now()
        status_text = (
            "📊 <b>Статус бота</b>\n"
            "━━━━━━━━━━━━━━━━━\n"
            f"🕐 Время сервера: <b>{now.strftime('%d.%m.%Y %H:%M:%S')}</b>\n"
            f"🗄 База данных: <b>{db_status}</b>\n"
            f"📅 Записей сегодня: <b>{today_count}</b>\n"
            f"📋 Всего активных: <b>{total_active}</b>\n"
