...
class MessageFormatter:
    ...
    @staticmethod
    def format_client_search_results(results: list[Appointment]) -> str:
        """BUG 8 FIX: общая функция форматирования результатов поиска клиентов.
        Используется в admin_handler.py и extended_features_handler.py вместо
        дублирующего кода формирования текста по каждому клиенту.
        """
        if not results:
            return "😔 Клиенты не найдены."
        lines = [f"🔍 Найдено {len(results)} клиент(ов):\n"]
        for appt in results:
            lines.append(MessageFormatter.admin_client_info(appt))
        return "\n\n".join(lines)
