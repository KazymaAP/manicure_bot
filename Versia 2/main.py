...
async def main() -> None:
    ...
    # BUG 18 FIX: проверяем валидность BOT_TOKEN при старте, а не при первом запросе.
    # При невалидном токене выдаём понятное сообщение и завершаем работу.
    try:
        import sys
        bot_info = await bot.get_me()
        logger.info("Бот авторизован: @%s (id=%s)", bot_info.username, bot_info.id)
    except Exception as token_exc:
        logger.critical(
            "❌ BOT_TOKEN невалиден или нет доступа к Telegram API: %s\n"
            "Проверьте значение BOT_TOKEN в .env файле.",
            token_exc,
        )
    ...
    # BUG 18 FIX: /status регистрируется только для администраторов через scope=BotCommandScopeChat
    admin_commands = [
        BotCommand(command="start", description="Начать работу с ботом"),
        BotCommand(command="help", description="Справка"),
        BotCommand(command="mybookings", description="Мои записи"),
        ...
        BotCommand(command="status", description="Статус бота (только для админов)"),
    ]
    for admin_id in settings.admin_ids:
        try:
            await bot.set_my_commands(admin_commands, scope=BotCommandScopeChat(chat_id=admin_id))
        except Exception as _cmd_exc:
            logger.debug("Не удалось установить команды для admin %s: %s", admin_id, _cmd_exc)
